 ...
 *
 * 		- Retorna os vetores que contem os expoentes locais, globais e as somas obtidos a cada iteracao;
 *
 ***********************************************************************************************************************
 * PENDENCIAS E MODIFICACOES:
 * 		- Passar "nMaxItera" como argumento do kernel;
 *
 * 		- Empregar "shared memory":
 * 			- O algoritmo começa rápido mas a medida que as iterações vão sendo contempladas, o algoritmo
 * 			fica cada vez mais lento atingindo um limiar de, aproximadamente, 0.17 seg por iteração;
 *
...
__global__ void processDuffing1989RK4_ClDyn(
		double * Tempo,
		double * outLyapLocal1, double * outLyapLocal2, double * outLyapLocal3,
		double * outLyapSoma1, 	double * outLyapSoma2, double * outLyapSoma3,
	...
	//		- Deve ser definido como constante 	;caso se queira empregar em indices de vetores
	...
	while (idxIteracoes < nMaxIteracoes) {
		...
		/* ************************************************************************************ */
		// ARMAZENA OS VALORES DO EXPOENTES A CADA ITERACAO
		outLyapLocal1[globalThreadIdx+idxIteracoes] = LyapLocal[0][idxIteracoes];
		outLyapLocal2[globalThreadIdx+idxIteracoes] = LyapLocal[1][idxIteracoes];
		outLyapLocal3[globalThreadIdx+idxIteracoes] = LyapLocal[2][idxIteracoes];

		outLyapSoma1[globalThreadIdx+idxIteracoes] = LyapSoma[0][idxIteracoes];
		outLyapSoma2[globalThreadIdx+idxIteracoes] = LyapSoma[1][idxIteracoes];
		outLyapSoma3[globalThreadIdx+idxIteracoes] = LyapSoma[2][idxIteracoes];

		outLyap1[globalThreadIdx+idxIteracoes] = Lyap[0][idxIteracoes];
		outLyap2[globalThreadIdx+idxIteracoes] = Lyap[1][idxIteracoes];
		outLyap3[globalThreadIdx+idxIteracoes] = Lyap[2][idxIteracoes];
		/* ************************************************************************************ */
		// ATUALIZA O VETOR TEMPO
		Tempo[globalThreadIdx+idxIteracoes] = tempo;
		/* ************************************************************************************ */
