/*
 * Versao inicial: 25/07/2013
 *
 * Ultima atualizacao: 30/08/2013
 *
 ***********************************************************************************************************************
 * DESCRICAO:
 ...
 *
 * 		- Retorna os expoentes locais medios, calculado no intervalo de tempo definido por "t_final". Os expoentes
 * 		locais médios são empregados na identiciação das LCS
 *
 *
 ************************************************************************************************************************
 * PENDENCIAS E MODIFICACOES:
 * 		- Passar "nMaxItera" e/ou "t_final" como argumento do kernel;
 * 			- Talvez procurar alguma coisa relacionada com alocação dinâmica
 *
 ...
 *
 * 			[4] Referencias sobre a construção de ondas quadradas em C:
...
__global__ void processDuffing1989RK4_ClDyn(
	...
	Lyap[2][0] = 0;
	/* **************************************************************************************** */
	// ALOCA MEMORIA PARA VARIAVEIS AUXILIARES EMPREGADAS NO CALCULO DOS EXPOENTES LOCAIS MEDIOS
	double nTempLyapLocal1 = 0;
	double nTempLyapLocal2 = 0;
	double nTempLyapLocal3 = 0;
	/* **************************************************************************************** */
	...
	/* ************************************************************************************ */
	// RETORNO DO KERNEL: EXPOENTE LOCAL MEDIO

	while( idxAux <= nMaxIteracoes ){

		nTempLyapLocal1 += LyapLocal[0][idxAux];
		nTempLyapLocal2 += LyapLocal[1][idxAux];
		nTempLyapLocal3 += LyapLocal[2][idxAux];

		idxAux++;

	}

	// Tem que somar '1' ao número máximo de iterações porque tem que contar que o indice começa em '0'
	outLyapLocal1[globalThreadIdx] = nTempLyapLocal1 / (double)(nMaxIteracoes+1);
	outLyapLocal2[globalThreadIdx] = nTempLyapLocal2 / (double)(nMaxIteracoes+1);
	outLyapLocal3[globalThreadIdx] = nTempLyapLocal3 / (double)(nMaxIteracoes+1);
