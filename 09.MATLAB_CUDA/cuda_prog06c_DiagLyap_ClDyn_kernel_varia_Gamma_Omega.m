...
%% ===================================================================== %%
%% NOME DO ARQUIVO:
%   - cuda_prog06c_DiagLyap_ClDyn_kernel_varia_Gamma_Omega.m
%
%
...
%
%   - Vairacao dos parametros: vOmega e vOmega;
%
%
%% OBSERVACOES:
%   - [14/10/2013]:
...
vGamma = 0.1:nIncControlParameter:1.0;
vOmega = 0.8:nIncControlParameter:1.6;


fprintf('| nIncParam = %.6f |\n', nIncControlParameter );
fprintf('| min(vGamma) = %.2f | max(vGamma) = %.2f |\n', min(vGamma), max(vGamma) );
...
% Variavel auxiliar
nComprimento = length(vCols_mGridGamma);
...
[ Lyap1, Lyap2, Lyap3 ] = feval( kernel, ...
    ...
    H, vCols_mGridGamma, vEpsilon, vCols_mGridOmega, ...
    numElementos);
...
% Vetor de dados
mStatesLocalLyapClDyn    = [ vCols_mGridGamma, vCols_mGridOmega, Lyap, max( Lyap, [], 2 ) ];
mNegStatesLocalLyapClDyn = [ vCols_mGridGamma, vCols_mGridOmega, Lyap, min( Lyap, [], 2 ) ];

% Vetor para construcao do espaco de estados
Mz    = reshape( mStatesLocalLyapClDyn(:,end),    size(mGridGamma,1), [] );
MzNeg = reshape( mNegStatesLocalLyapClDyn(:,end), size(mGridGamma,1), [] );
...
figure(2);
surf(mGridGamma, mGridOmega, MzNeg);
colormap jet;
shading interp;
view([0 90]);
