% Ultima atualizacao: 28/03/2014



%% ===================================================================== %%
%% NOME DO ARQUIVO:
%   - cuda_prog06b_DiagLyap_ClDyn_kernel_varia_Epsilon_Omega.m
%
%
...
%
%   - Vairacao dos parametros: vEpsilon e vOmega;
%
%
%% OBSERVACOES:
%   - [14/10/2013]:
...
nIncControlParameter = 0.0005;

% Parametros de controle
% vGamma = [0.15, 0.18, 0.22, 0.25];
% vOmega = [0.1, 0.3];

vEpsilon = 0.0:nIncControlParameter:0.5;
vOmega   = 0.9:nIncControlParameter:1.3;


fprintf('| nIncParam = %.6f |\n', nIncControlParameter );
fprintf('| min(vEpsilon) = %.2f | max(vEpsilon) = %.2f |\n', min(vEpsilon), max(vEpsilon) );
fprintf('| min(vOmega)   = %.2f | max(vOmega)   = %.2f |\n', min(vOmega), max(vOmega) );
fprintf('\n');
...
vCols_mGridOmega   = reshape( mGridOmega, [], 1 );
...
eval(['!/usr/local/cuda-5.0/bin/nvcc -ptx cudaDuffing1989RK4_DiagLyap_v01.cu']);
...
[ Lyap1, Lyap2, Lyap3 ] = feval( kernel, ...
    ...
    H, vGamma, vCols_mGridEpsilon, vCols_mGridOmega, ...
    numElementos);
...
% Vetor de dados
mStatesLocalLyapClDyn    = [ vCols_mGridEpsilon, vCols_mGridOmega, Lyap, max( Lyap, [], 2 ) ];
mNegStatesLocalLyapClDyn = [ vCols_mGridEpsilon, vCols_mGridOmega, Lyap, min( Lyap, [], 2 ) ];
...
strEpsilon = ['vEpsilon_' num2str(min(vEpsilon)) '_a_' num2str(max(vEpsilon)) ];
strEpsilon( ismember(strEpsilon, '-') ) = 'n';
strEpsilon( ismember(strEpsilon, '.') ) = 'p';

strGamma = ['vGamma_' num2str(min(vGamma)) '_a_' num2str(max(vGamma)) ];
strGamma( ismember(strGamma, '-') ) = 'n';
strGamma( ismember(strGamma, '.') ) = 'p';

strOmega = ['vOmega_' num2str(min(vOmega)) '_a_' num2str(max(vOmega)) ];
strOmega( ismember(strOmega, '-') ) = 'n';
strOmega( ismember(strOmega, '.') ) = 'p';

strPasso = ['nIncParam_' num2str(nIncControlParameter) ];
strPasso( ismember(strPasso, '-') ) = 'n';
strPasso( ismember(strPasso, '.') ) = 'p';
...
sSave = ['save dados_Duffing1989_DiagLyap_' strEpsilon '_' strGamma '_' strOmega '_tf_' num2str(t_final) '_' strPasso '_GPU_CUDA_Kernel_' stringnameGPU '.mat'];
eval(sSave);
fprintf ( 1, '  done!\n\n');
...
figure(1);
surf(mGridEpsilon, mGridOmega, Mz);
colormap jet;
shading interp;
view([0 90]);
...
xlim([ min(vEpsilon) max(vEpsilon) ]);
ylim([ min(vOmega) max(vOmega) ]);

%title( ['[epsilon = ', num2str(epsilon), '][gamma = ', num2str(gamma), ']'] );

sGraficoEPS = ['print -depsc2 fig_Duffing1989_DiagLyap_' strEpsilon '_' strGamma '_' strOmega '_tf_' num2str(t_final) '_' strPasso '_GPU_CUDA_Kernel_' stringnameGPU '_Mz.eps'];
eval(sGraficoEPS);

% ----------------------------------------------------------------------- %

figure(2);
surf(mGridEpsilon, mGridOmega, MzNeg);
colormap jet;
shading interp;
view([0 90]);
...
sGraficoEPS = ['print -depsc2 fig_Duffing1989_DiagLyap_' strEpsilon '_' strGamma '_' strOmega '_tf_' num2str(t_final) '_' strPasso '_GPU_CUDA_Kernel_' stringnameGPU '_MzNeg.eps'];
eval(sGraficoEPS);
