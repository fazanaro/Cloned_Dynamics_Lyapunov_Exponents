 ...
 *
 * 		- Retorna os expoentes globais;
 *
 *
 ************************************************************************************************************************
 * PENDENCIAS:
 * 		- Passar "nMaxItera" e/ou "t_final" como argumento do kernel;
 * 			- Talvez procurar alguma coisa relacionada com alocacao dinamica
 *
 * 		- Empregar "shared memory":
 * 			- O algoritmo comeca rapido mas a medida que as iteracoes vao sendo contempladas, o algoritmo
 * 			fica cada vez mais lento atingindo um limiar de, aproximadamente, 0.17 seg por iteracao;
 *
 *
 ************************************************************************************************************************
 * MODIFICACOES:
 * 		- Esse codigo eh o mesmo daquele implementado para a obtencao das LCS no caso do modelo de Duffing bi-dimensional,
 * 		i.e., os expoentes sao acumulados e, ao final, eh feita a media no intervalo de tempo considerado;
 * 		
 * 		- [21/03/2014]:
 * 			- Adiciona o comando "__syncthreads();" antes do retorno do kernel;
 * 
 * 
 ************************************************************************************************************************
 * REFERENCIAS
 ...
 */
__global__ void processDuffing1989RK4_ClDyn_Diagrama_Lyap(
	...
	/* ************************************************************************************ */
	// RETORNO DO KERNEL: EXPOENTES

	outLyap1[globalThreadIdx] = LyapSoma[0][0] / (t_final - t_init);
	outLyap2[globalThreadIdx] = LyapSoma[1][0] / (t_final - t_init);
	outLyap3[globalThreadIdx] = LyapSoma[2][0] / (t_final - t_init);
