...
%       - alpha variation step: 0.05
%       - Total execution time [s] = 341.7621
%
%       - alpha variation step: 0.01
%       - Total execution time [s] = 16366.7943
%
%
%% ===================================================================== %%
%% REFERENCES
...
% Maximal number of points in each Poincare section
nMaxPoincare = 150;

% Memory allocation
%   - The first column will contain the parameter value for each iteration
vPoincareX = 255*ones(length(vAlpha),(nMaxPoincare+1));
vPoincareY = 255*ones(length(vAlpha),(nMaxPoincare+1));
...
for iAlpha = 1:length(vAlpha)
    ...
    %   (4) Estimates the value of the state variables (in this case, 'x'
    %   and 'y') for the section
...
%                 (varZ(ii) <= Threshold)&&(varZ(ii+1) >= Threshold)
    ...
    for ii = 1:length(index)
        ...
        % --------------------------------------------------------------- %
        % SAMPLE 'X'
        % Calculate the curve coefficients associate to the 'x'
        % interpolation 
        ...
        % --------------------------------------------------------------- %
        % SAMPLE 'Y'
        % Calculate the curve coefficients associate to the 'y'
        % interpolation 
    ...
    % ------------------------------------------------------------------- %
    % STORE THE POINTS FOR THIS POINCARE SECTION
    
    vPoincareX(iAlpha,1) = alpha;
    vPoincareX(iAlpha,2:(nMaxPoincare+1)) = PoincareX(1:nMaxPoincare);
%     vPoincareX(iAlpha,2:4) = PoincareX(1:3);
    
    vPoincareY(iAlpha,1) = alpha;
    vPoincareY(iAlpha,2:(nMaxPoincare+1)) = PoincareY(1:nMaxPoincare);
%     vPoincareY(iAlpha,2:4) = PoincareY(1:3);
    % ------------------------------------------------------------------- %
...
sSave = ['save data_bifurc_PoincareZ_ChuaAdim1985_vAlpha_' num2str(vAlpha(1)) 'a' num2str(vAlpha(end)) '.mat'];
...
plot(vPoincareX(:,1),vPoincareX(:,2:end), ...
    'LineStyle','none', ...
    'Marker','.', ...
...
plot(vPoincareY(:,1),vPoincareY(:,2:end), ...
    'LineStyle','none', ...
    'Marker','.', ...
    'MarkerSize', MarkerSize, ...
    'Color',[0 0 0]);
